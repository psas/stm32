...
int main(void) {
    ...
    struct multicast_lwipthread_opts   ip_opts;
    ...
    static       uint8_t      UNICAST_macAddress[6]         = UNICAST_MAC_ADDRESS;
    struct       ip_addr      ip, gateway, netmask;
    UNICAST_IP_ADDR(&ip);
    UNICAST_GATEWAY(&gateway);
    UNICAST_NETMASK(&netmask);

    ip_opts.macaddress = UNICAST_macAddress;
    ip_opts.address    = ip.addr;
    ip_opts.netmask    = netmask.addr;
    ip_opts.gateway    = gateway.addr;

	 struct       ip_addr	multicast_ip, multicast_mask;
	 MULTICAST_MOST_ADDRESS(&multicast_ip);
	 MULTICAST_NETMASK(&multicast_mask);
	 ip_opts.multicast_addr_1 = multicast_ip.addr;
	 ip_opts.multicast_mask_1 = multicast_mask.addr;
    chThdCreateStatic(wa_lwip_thread              , sizeof(wa_lwip_thread)              , NORMALPRIO + 2, multicast_lwip_thread            , &ip_opts);
