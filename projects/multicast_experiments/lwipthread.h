...
#if !defined(LWIP_MULTICAST_ADDR) || defined(__DOXYGEN__)
#define LWIP_MULTICAST_ADDR(p)					IP4_ADDR(p, 224, 0, 0, 1)
#endif

/** @brief multicast netmask */
#if !defined(LWIP_MULTICAST_NETMASK) || defined(__DOXYGEN__)
#define LWIP_MULTICAST_NETMASK(p)				IP4_ADDR(p,255,255,255,255)
#endif
 ...
 */
struct multicast_lwipthread_opts {
  uint8_t       *macaddress;
  uint32_t      address;
  uint32_t      netmask;
  uint32_t      gateway;
  uint32_t		 multicast_addr_1;
  uint32_t		 multicast_mask_1;
  uint32_t		 multicast_addr_2;
  uint32_t		 multicast_mask_2;
};
