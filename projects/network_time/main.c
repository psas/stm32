...
/*!
 * \defgroup mainapp network time experiments
 * @{
 */
...
int main(void) {
   ...
   static       uint8_t      IMU_macAddress[6]           = TIME_CLIENT_MAC_ADDRESS;
   struct       ip_addr      ip, gateway, netmask;
   TIME_CLIENT_IP_ADDR(&ip);
   TIME_CLIENT_GATEWAY(&gateway);
   TIME_CLIENT_NETMASK(&netmask);
   ...
   chThdCreateStatic(wa_lwip_thread            , sizeof(wa_lwip_thread)            , NORMALPRIO + 2, lwip_thread            , &ip_opts);
	chThdCreateStatic(wa_data_udp_time_control_send_thread, sizeof(wa_data_udp_time_control_send_thread), NORMALPRIO, data_udp_time_control_send, NULL);
	chThdCreateStatic(wa_data_udp_time_control_receive_thread, sizeof(wa_data_udp_time_control_receive_thread), NORMALPRIO, data_udp_time_control_receive, NULL);
